...
class CategoryPage(BasePage):
    ...
    def switch_to_list_view(self):
        ...
        button = self.wait.until(
            EC.presence_of_element_located(
                CategoryLocators.LIST_VIEW_BTN
            )
        )
        ...
        self.wait.until(
            lambda driver: "o_wsale_layout_list" in driver.page_source
        )

    def select_steel_filter(self):
        checkbox = self.wait.until(
            EC.presence_of_element_located(
                CategoryLocators.STEEL_CHECKBOX
            )
        )
        ...
        self.wait.until(
            EC.url_contains("attrib=1-1")
        )

    def click_cart_icon(self):
        icon = self.wait.until(
            EC.element_to_be_clickable(
                CategoryLocators.ADD_TO_CART_ICON
            )
        )
    ...
    def proceed_to_checkout(self):
        btn = self.wait.until(
            EC.element_to_be_clickable(
                CategoryLocators.PROCEED_CHECKOUT_BTN
            )
        )
        btn.click()
    ...
    def should_be_on_cart_page(self):
        self.wait.until(
            EC.url_contains("/shop/cart")
        )

    def verify_product_in_cart(self, full_name, description):
        title = self.get_text(
            CategoryLocators.CART_PRODUCT_TITLE
        )

        desc = self.get_text(
            CategoryLocators.CART_PRODUCT_DESCRIPTION
        )
